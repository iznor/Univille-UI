...
export const userActions = {
    ...
    tgAuth: (phoneNumber, password) => {
        return async (dispatch,getState) => {
            const {user:{user:{email}}} = getState();
            try {
                dispatch(uiActions.setLoader(true));
                const response = await authApi.tgAuth(phoneNumber, password,email);
    ...
    tgVerify: (phoneNumber, code) => {
        return async dispatch => {
            try {
                dispatch(uiActions.setLoader(true));
                const response = await authApi.tgVerify(phoneNumber, code);
                const {token, user} = response;
